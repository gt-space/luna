...
use common::comm::{
    flight::DataMessage,
    sam::{SamControlMessage, SamDataPoint},
    ValveAction,
};
use hostname::get;
...
use crate::{
    command::{check_valve_abort_timers, execute},
    state::{AbortInfo, ConnectData},
    SamVersion, CACHED_FC_ADDRESS, FC_ADDR,
};
...
pub fn get_version() -> SamVersion {
    ...
    {
        SamVersion::Rev3
    } else if name == "sam-11" || name == "sam-12" || name == "sam-13" || name == "sam-14" {
...
pub fn establish_flight_computer_connection(
    data: &mut ConnectData,
) -> (UdpSocket, UdpSocket, SocketAddr, String, AbortInfo) {
    ...
    // look for the flight computer based on it's dynamic IP. if we have
    // previously connected to the flight computer, we cache the address we
    // connected to and upon attempted reconnection only use that address. this
    // significantly reduces DNS delays during reconnects, which are critical to
    // meeting the timing of abort timers.
...
pub fn check_heartbeat(
    ...
    match data_socket.recv_from(&mut data_buffer) {
        Ok((size, _)) => match postcard::from_bytes::<DataMessage>(&data_buffer[..size]) {
    ...
    match command_socket.peek_from(&mut command_buffer) {
        Ok((size, _)) => {
            match postcard::from_bytes::<SamControlMessage>(&command_buffer[..size]) {
                ...
                Err(e) => {
                    warn!("Could not deserialize command from FC ({e}), continuing...")
                }
            }
        }
...
pub fn check_and_execute(
    command_socket: &UdpSocket,
    abort_info: &mut AbortInfo,
    abort_valve_states: &mut Vec<(ValveAction, bool)>,
) {
