...
pub struct ValveSafeState {
    ...
    /// Timer (in milliseconds!!!) that allows us to delay putting a valve in
    /// its safe state by some amount of time Can't use Instant here since
    /// Instant does not implement serde::Serialize or deserialize
...
pub enum SequenceDomainCommand {
    ...
    CreateAbortStage {
        ...
        /// Can use the python eval builtin to run strings as code
        abort_condition: String,
        /// Desired states of valves that we want to go to in an abort during
        /// this stage
    ...
    /// Tells the FC whether to monitor servo disconnects (and react) or ignore
    /// them.
    ///
    /// When `enabled` is false, the FC should not abort on servo disconnect
    /// timeouts, and once a disconnect is detected it should stop attempting
    /// to reconnect to, pull from, or push telemetry to the servo process. If
    /// enabled is true, the FC should monitor servo disconnects.
    ...
    /// Start the CTV control loop and record the current time as the start
    /// time.
    CtvStartFlight,
