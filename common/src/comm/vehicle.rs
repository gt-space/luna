...
use super::{
    bms::Bms, fc_sensors::FcSensors, rbf::RbfState, sam, AbortStage, CompositeValveState, GpsState,
    Measurement, RecoState, Statistics,
};
...
pub enum VehicleStateSchemaError {
    ...
    TooManyValves,
    /// The number of serialized sensors exceeds what this wire format can
    /// represent.
    TooManySensors,
}
...
impl VehicleStateCompressionSchema {
    ...
    pub fn new(
        ...
        u8::try_from(valve_keys.len()).map_err(|_| VehicleStateSchemaError::TooManyValves)?;

        let mut sensor_keys: Vec<_> = sensor_keys.into_iter().collect();
        sensor_keys.sort_unstable();
        u8::try_from(sensor_keys.len()).map_err(|_| VehicleStateSchemaError::TooManySensors)?;
...
impl VehicleStateDecompressionSchema {
    ...
    pub fn new(
        ...
        let (sensor_keys, sensor_units): (Vec<_>, Vec<_>) = sensor_metadata.into_iter().unzip();
...
/// Implements all fields as default except for the AbortStage field whose name
/// becomes "default"
...
fn average_reco_states(reco: &RecoTriState) -> Option<RecoState> {
    ...
    fn average_scalar(present: &[&RecoState], select: impl Fn(&RecoState) -> f32) -> f32 {
...
impl VehicleState {
    ...
    /// Serializes this vehicle state using the compaq-derived TEL
    /// representation.
    pub fn compress_compaq(&self, buf: &mut [u8]) -> Result<usize, VehicleStateCompaqError> {
    ...
    /// Reconstructs a `VehicleState` from the compaq-derived TEL
    /// representation.
...
mod tests {
    use super::{
        super::{fc_sensors, sam::Unit, ValveState},
        *,
    };

    fn vehicle_state_with_counts(valve_count: usize, sensor_count: usize) -> VehicleState {
    ...
    #[test]
    /// For the use case of a test, it is much easier to manually assign values
    /// to the fields of a struct (VehicleState) rather than using the
    ...
    fn compaq_vespula_size_breakdown() {
        fn print_breakdown(label: &str, state: &VehicleState) {
            let schema = VehicleStateCompressionSchema::from_state(state)
                .expect("schema creation should succeed");
            let compressed =
                match state.deflate(schema.valve_keys.clone(), schema.sensor_keys.clone()) {
