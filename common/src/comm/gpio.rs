...
use rppal::gpio::{Gpio as RpiGpio, InputPin as RpiInputPin, Level, OutputPin as RpiOutputPin};
...
impl RpiGpioController {
    ...
    /// Returns a pin for the given BCM pin number. The pin is unconfigured
    /// until you call `mode()`. The returned pin borrows this controller.
...
impl<'a> GpioPin for RpiPin<'a> {
    ...
    fn digital_read(&self) -> PinValue {
        match &self.inner {
            ...
            // changing the interface of the trait and i am not sure if that is worth
            // it
...
impl Gpio {
    ...
    pub fn open_controller(controller_index: usize) -> Gpio {
        ...
        let direction = Mutex::new(unsafe { base.offset(GPIO_OE_REGISTER) as *mut u32 });

        let dataout = Mutex::new(unsafe { base.offset(GPIO_DATAOUT_REGISTER) as *mut u32 });

        let datain = Mutex::new(unsafe { base.offset(GPIO_DATAIN_REGISTER) as *const u32 });
