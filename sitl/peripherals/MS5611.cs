...
namespace Antmicro.Renode.Peripherals.Sensors
{
    ...
    IProvidesRegisterCollection<WordRegisterCollection>
  {
    ...
    public byte Transmit(byte data)
    {
      ...
      {
        command = data;
        Console.WriteLine($"Received command: 0x{data:X2}"); // X2 converts data to 2 digit hexadecimal
        byteIndex = 0;
        return 0xFF;
      }
      ...
      else if ((command & 0xF0) == 0xA0) // PROM read command
      {
        ...
        byte msb = (byte)RegistersCollection.Read(address + 1);
        byte lsb = (byte)RegistersCollection.Read(address);
        ...
        {
          ushort[] proms = new ushort[] { RegistersCollection.Read(0x02), RegistersCollection.Read(0x04), RegistersCollection.Read(0x06), RegistersCollection.Read(0x08), RegistersCollection.Read(0x0A), RegistersCollection.Read(0x0C) };
          return crc4(proms);
        }
      ...
      Console.WriteLine($"CMD={command:X2} IDX={byteIndex} RESP={response:X2}");
      return response;
    }
