...
impl Lab {
    ...
    pub fn create_bridge_link(
        ...
        run_command(&[
            "ip", "link", "add", pair.left, "type", "veth", "peer", "name", pair.right,
        ])?;
    ...
    pub fn rename_interface(&self, namespace: &str, from: &str, to: &str) -> Result<()> {
        run_command(&[
            "ip", "netns", "exec", namespace, "ip", "link", "set", from, "name", to,
        ])
    }

    pub fn set_interface_mtu(&self, namespace: &str, interface: &str, mtu: &str) -> Result<()> {
        run_command(&[
            "ip", "netns", "exec", namespace, "ip", "link", "set", interface, "mtu", mtu,
        ])
    }

    pub fn add_addr(&self, namespace: &str, addr: &str, interface: &str) -> Result<()> {
        run_command(&[
            "ip", "netns", "exec", namespace, "ip", "addr", "add", addr, "dev", interface,
        ])
    }
