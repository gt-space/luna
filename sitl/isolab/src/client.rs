...
pub fn count_valve_helper_sensors(mappings: &[NodeMapping]) -> usize {
    mappings
        ...
        .filter(|mapping| mapping.text_id.ends_with("_I") || mapping.text_id.ends_with("_V"))
        .count()
}
...
pub fn count_non_radio_mappings(mappings: &[NodeMapping]) -> usize {
    mappings
        .iter()
        .filter(|mapping| {
            !mapping.board_id.starts_with("sam-2") && !mapping.board_id.starts_with("sam-3")
        })
        .count()
}
...
pub async fn next_vehicle_state(
...
pub async fn wait_for_repeated_state(
    socket: &mut ServoSocket,
    deadline: Duration,
) -> Result<VehicleState> {
    let end = Instant::now() + deadline;
