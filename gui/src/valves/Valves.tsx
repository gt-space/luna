...
listen('device_update', (event) => {
  ...
  valveDevices.forEach(async (device) => {
    var index = valves().findIndex(
      (item) => item.name === (device[0] as string)
    );
    var new_valves = [...valves()];
    console.log(device[1]);
    var valveStates = device[1] as unknown as object;
    new_valves[index].commanded =
      valveStates['commanded' as keyof typeof valveStates];
    new_valves[index].actual =
      valveStates['actual' as keyof typeof valveStates];
...
listen('state', (event) => {
  ...
  var activeconfmappings = (configurations() as Config[]).filter((conf) => {
...
function toggleSequenceButton() {
  ...
  const seqDropdown = document.getElementById(
    'sequenceselect'
    ...
    <div class="window-template">
      ...
      <div class="valve-view">
        <div style={{ display: 'flex' }}>
          <div id="valvebutton" class="addvalvebutton" onClick={openDropdown}>
            Add/Remove Valves
          </div>
          <div id="valvedropdown" class="valvedropdowncontent">
            {deviceOptions().length != 0 ? (
              <For each={deviceOptions() as Mapping[]}>
                {(mapping, i) => (
                  <div
                    class="valvedropdownitem"
                    onClick={() => addValveDevice(mapping)}
                  >
                    {mapping.text_id}
                  </div>
                )}
              ...
              <div class="valvedropdownitem">
                no valves or active config rip
              </div>
            )}
          </div>
        </div>
        <div class="valve-view-section">
          <div
            style={{
              display: 'grid',
              'grid-template-columns': '4fr 10fr 220px',
            }}
          >
          ...
          <For each={valves()}>
            {(valve, i) => (
              <div class="valve-row">
                ...
                <div
                  style="width: 10px; height: 30px; border-left-style:solid; 
            border-left-color: #737373; border-left-width: 1px"
                ></div>
                <button
                  class="valve-button"
                  style={{ 'background-color': '#22873D' }}
                  onClick={() => openValve(valves()[i()].name)}
                >
                  Open
                </button>
                <button
                  ...
                  onClick={() => closeValve(valves()[i()].name)}
                >
                  Close
                </button>
                <div
                  style="width: 10px; height: 30px; border-right-style:solid; 
            border-right-color: #737373; border-right-width: 1px"
                ...
                <div
                  style={{
                    ...
                    height: '10px',
                    padding: '5px',
                    'background-color': stateToColor(valves()[i()].commanded),
                  }}
                >
                  {valves()[i()].commanded.charAt(0).toUpperCase() +
                    valves()[i()].commanded.substring(1)}
                </div>
                <div
                  style={{
                    display: 'flex',
                    ...
                    height: '10px',
                    padding: '5px',
                    'background-color': stateToColor(valves()[i()].actual),
                  }}
                >
                  {valves()[i()].actual.charAt(0).toUpperCase() +
                    valves()[i()].actual.substring(1)}
                </div>
              </div>
            )}
          </For>
