...
listen('device_update', (event) => {
  ...
  var devices = Object.keys(sensor_object).map((key) => [
    key,
    ...
    <div class="window-template">
      ...
      <div style="display: flex; flex-direction: column; overflow: hidden">
        <div style="display: flex; justify-content: center; gap: 20px">
          <div id="sensbutton" class="addsensbutton" onClick={openDropdown}>
            Add/Remove Sensors
          </div>
          <div id="sensdropdown" class="sensdropdowncontent">
            {deviceOptions().length != 0 ? (
              <For each={deviceOptions() as Mapping[]}>
                {(mapping, i) => (
                  <div
                    class="sensdropdownitem"
                    onClick={() => addSensDevice(mapping)}
                  >
                    {mapping.text_id}
                  </div>
                )}
              ...
              <div class="sensdropdownitem">
                no sensors or active config rip
              </div>
            )}
          </div>
          <button class="toggle-view-button" onClick={calibrate}>
            Calibrate
          </button>
        </div>
