...
function displayView(view: string, sensors: Device[]) {
  switch (view) {
    case 'sorted':
      return <SortableSensorView sensors={sensors} />;
    case 'grouped': {
      let fuel: Device[] = [];
      let oxygen: Device[] = [];
      ...
      for (var sensor in sensors) {
        switch (sensors[sensor].group.toLowerCase()) {
          case 'fuel':
            fuel.push(sensors[sensor]);
            break;
          case 'oxygen':
            oxygen.push(sensors[sensor]);
            break;
          case 'pressurant':
            pressurant.push(sensors[sensor]);
            break;
        }
      }
      return (
...
const SensorSectionView: Component<{ sensors: Device[] }> = (props) => {
  return (
    <div style="display: flex; flex-direction: column; flex:1">
      {displayView(view(), props.sensors)}
    </div>
  );
};
