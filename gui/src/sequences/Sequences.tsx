    ...
    <div class="window-template">
      ...
      <div class="sequences-view">
        <div class="sequences-panel">
          ...
          >
            <For each={sequences() as Sequence[]}>
              {(sequence, i) => (
                <option class="seq-dropdown-item" value={sequence.name}>
                  {sequence.name}
                </option>
              )}
            </For>
          ...
          <div style={{ flex: 1, padding: '5px' }}>
            <button
              class="toggle-sequence-button"
              id="sequencebutton"
              onClick={dispatchSequence}
            >
              Dispatch Sequence
            </button>
          </div>
        ...
        >
          <button
            class="abort-button"
            onClick={() => sendAbort(serverIp() as string)}
          >
            {' '}
            ABORT{' '}
          </button>
        ...
        <div class="sequences-view-section">
          <For each={runningSeqs() as Array<string>}>
            {(seq, i) => (
              <div class="running-seq-row">
                <div>{seq}</div>
                <button
                  class="cancel-seq-button"
                  onClick={() => stopSequence(serverIp() as string, seq)}
                ></button>
              </div>
            )}
          </For>
