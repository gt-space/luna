...
const ChartComponent: Component<{ id: string; index: number }> = (props) => {
  ...
  createEffect(async () => {
    ...
    const myChart = new Chart(
      document.getElementById(props.id) as HTMLCanvasElement,
      config
    );
  ...
  return (
    <div>
      <button class="chart-reset-button" onClick={resetChartZoom}>
        Reset Zoom
      </button>
      <div style="display: block; margin: 0px; width: 400px;">
        <canvas id={props.id} class="chart-tile"></canvas>
      </div>
    </div>
  );
};
