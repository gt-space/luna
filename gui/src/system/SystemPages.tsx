...
function deleteAbortStageEntry(entry: AbortStageMapping) {
  ...
  var mappingabortstages = document.querySelectorAll("[id=addabortstage]") as unknown as Array<HTMLInputElement>;
  var mappingtimers = document.querySelectorAll("[id=addabortstagetimer]") as unknown as Array<HTMLSelectElement>;
  for (var i = 0; i < entries.length; i++) {
    entries[i].valve_name = mappingnames[i].value;
    entries[i].abort_stage = mappingabortstages[i].value === "N/A"? 
      null : JSON.parse(mappingabortstages[i].value.toLowerCase())
...
async function refreshAbortStages() {
  ...
  var abortStageList = await getAbortStages(ip);
  console.log(abortStageList);
  if (abortStageList instanceof Error) {
    setRefreshAbortStageDisplay('Error!');
    await new Promise(r => setTimeout(r, 1000));
    setRefreshAbortStageDisplay('Refresh');
    return;
  }
  var abortStageMap = new Map(Object.entries(abortStageList));
  var abortStageArray = Array.from(abortStageMap, ([name, value]) => ({'id': name, 'mappings': value }));
...
async function removeAbortStage(abortStageId: string) {
  const success = await deleteAbortStage(serverIp() as string, abortStageId) as object;
  const statusCode = success['status' as keyof typeof success];
  if (statusCode != 200) {
    refreshAbortStages();
    setSaveAbortStageDisplay("Error!");
    ...
    <div style={{"max-height": '100%', "overflow-y": "auto"}}>
      <For each={editableAbortStageEntries()}>{(entry, i) =>
          <div class="add-config-configurations">
            ...
            <select name="" id={"addabortstage"} value={entry.abort_stage.toUpperCase()} class="add-config-styling">
              <option class="seq-dropdown-item">N/A</option>
