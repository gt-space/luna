...
const [channelTypes, setChannelTypes] = createSignal([
  'PT',
  'VALVE',
  'FLOW METER',
  'RAIL VOLTAGE',
...
async function connectToServer() {
  ...
  var ip = (
    document.getElementsByName('server-ip')[0] as HTMLInputElement
  ).value.trim();
  var result = '';
...
function closeSessionId(evt: MouseEvent) {
  try {
    ...
    if (
      (evt.target as HTMLElement).id != popup.id &&
      (evt.target as HTMLElement)!.className != 'id-display'
    ) {
        ...
        <div class="system-connect-section">
          <div style="display: grid; grid-template-columns: 1fr 1fr">
            ...
            <div style="display: flex; flex-direction: column; margin-left: 0px">
              <div style="text-align: center" id="activity">
                {activity()} ms
              </div>
              ...
              <div id="session-id" style="text-align: center">
                {sessionId() == 'None' ? (
                  (sessionId() as string)
                ) : (
                  <Show
              ...
              <div style="text-align: center" id="forwarding-id">
                {forwardingId() == 'None' ? (
                  (forwardingId() as string)
                ) : (
                  <Show
...
async function setFeedsystemAndActiveConfig() {
  var feedsystem = document.querySelector(
    'input[name="feedsystem-select"]:checked'
  )! as HTMLInputElement;
  ...
  await invoke('update_feedsystem', {
    window: appWindow,
    value: feedsystem.value,
  });
...
async function setFeedsystemData() {
  ...
  var feedsystemToSet = document.querySelectorAll(
    'input[value="' + (feedsystem() as string) + '"]'
  )[0];
  var dropdown = document.getElementById(
    'feed-config-drop-1'
  )! as HTMLSelectElement;
...
async function refreshConfigs() {
  ...
  var configArray = Array.from(configMap, ([name, value]) => ({
    id: name,
    mappings: value,
  }));
  ...
  return (
    <div style="height: 100%; display: flex; flex-direction: column">
      ...
      <div class="select-feedsystem-body">
            ...
            >
              ...
              >
                <input
                  ...
                  style={{ margin: '10px' }}
                  type="radio"
                  name="feedsystem-select"
                  value="Feedsystem_1"
                  id="Feedystem-id-1"
                  checked
                ></input>
                <div>Feedsystem 1</div>
              </div>
              <div
                style={{
                  display: 'flex',
          ...
          <div style={{ flex: 2, 'padding-top': '20px' }}>
            ...
            >
              <div>
                ...
                >
                  <For each={configurations() as Config[]}>
                    {(config, i) => (
                      <option class="conf-dropdown-item" value={config.id}>
                        {config.id}
                      </option>
                    )}
                  </For>
        ...
        >
          <button
            class="submit-feedsystem-button"
            onClick={setFeedsystemAndActiveConfig}
          >
            {' '}
            Submit{' '}
          </button>
...
function deleteConfigEntry(entry: Mapping) {
  ...
  var entries = [...editableEntries()];
  var mappingnames = document.querySelectorAll(
    '[id=addmappingname]'
  ) as unknown as Array<HTMLInputElement>;
  var mappingboardids = document.querySelectorAll(
    '[id=addmappingboardid]'
  ) as unknown as Array<HTMLInputElement>;
  var mappingchanneltypes = document.querySelectorAll(
    '[id=addmappingchanneltype]'
  ) as unknown as Array<HTMLSelectElement>;
  var mappingchannels = document.querySelectorAll(
    '[id=addmappingchannel]'
  ) as unknown as Array<HTMLInputElement>;
  var mappingcomputers = document.querySelectorAll(
    '[id=addmappingcomputer]'
  ) as unknown as Array<HTMLSelectElement>;
  var mappingmins = document.querySelectorAll(
    '[id=addmappingmin]'
  ) as unknown as Array<HTMLSelectElement>;
  var mappingmaxs = document.querySelectorAll(
    '[id=addmappingmax]'
  ) as unknown as Array<HTMLSelectElement>;
  var mappingvalveconnecteds = document.querySelectorAll(
    '[id=addmappingvalveconnected]'
  ) as unknown as Array<HTMLSelectElement>;
  var mappingvalvepowereds = document.querySelectorAll(
    '[id=addmappingvalvepowered]'
  ) as unknown as Array<HTMLSelectElement>;
  var mappingvalvenormcloseds = document.querySelectorAll(
    '[id=addmappingvalvenormclosed]'
  ) as unknown as Array<HTMLSelectElement>;
  for (var i = 0; i < entries.length; i++) {
    entries[i].text_id = mappingnames[i].value;
    entries[i].board_id = mappingboardids[i].value;
    entries[i].sensor_type = mappingchanneltypes[i].value
      .replace(' ', '_')
      .toLowerCase();
    ...
    entries[i].min =
      mappingmins[i].value === ''
        ? NaN
        : (mappingmins[i].value as unknown as number);
    entries[i].max =
      mappingmaxs[i].value === ''
        ? NaN
        : (mappingmaxs[i].value as unknown as number);
    entries[i].powered_threshold =
      mappingvalvepowereds[i].value === ''
        ? NaN
        : (mappingvalvepowereds[i].value as unknown as number);
    entries[i].normally_closed =
      mappingvalvenormcloseds[i].value === 'N/A'
        ? null
        : JSON.parse(mappingvalvenormcloseds[i].value.toLowerCase());
  }
  console.log(entry);
...
async function submitConfig(edited: boolean) {
  var newConfigNameInput = (document.getElementById(
    'newconfigname'
  ) as HTMLInputElement)!;
  var configName;
  if (edited) {
  ...
  const success = (await sendConfig(
    serverIp() as string,
    { id: configName, mappings: entries } as Config
  )) as object;
    ...
    <div style={{ width: '100%' }}>
      <div class="add-config-section">
        <div class="add-config-setup">
          ...
          <input
            id="newconfigname"
            class="add-config-input"
            type="text"
            placeholder="Name"
          />
        </div>
        <div class="add-config-btns">
          <button class="add-config-btn" onClick={addNewConfigEntry}>
            Insert Mapping
          </button>
          <button
            style={{ 'background-color': '#C53434' }}
      ...
      <div
        ...
        class="add-config-configurations"
      >
          ...
          {(entry, i) => (
            <div class="add-config-configurations">
              <input
                ...
                value={entry.text_id}
                placeholder="Name"
                class="add-config-styling"
              />
              <input
                type="text"
                name=""
                id={'addmappingboardid'}
                value={entry.board_id}
              ...
              <select
                name=""
                id={'addmappingchanneltype'}
                value={entry.sensor_type.toUpperCase()}
                class="add-config-styling"
              >
              ...
              <input
                ...
                name=""
                id={'addmappingchannel'}
                value={Number.isNaN(entry.channel) ? '' : entry.channel}
                placeholder="Channel"
                class="add-config-styling"
              />
              <select
                name=""
                id={'addmappingcomputer'}
                value={entry.computer as string}
                class="add-config-styling"
              >
              ...
              <input
                ...
                name=""
                id={'addmappingmin'}
                value={Number.isNaN(entry.min) ? '' : entry.min}
                placeholder="Min"
                class="add-config-styling"
              />
              <input
                ...
                name=""
                id={'addmappingmax'}
                value={Number.isNaN(entry.max) ? '' : entry.max}
                placeholder="Max"
                class="add-config-styling"
              />
              <input
                ...
                id={'addmappingvalvepowered'}
                value={
                  Number.isNaN(entry.powered_threshold)
                    ? ''
                    : entry.powered_threshold
                }
                placeholder="ValvePowThresh"
                class="add-config-styling"
              />
              <select
                name=""
                id={'addmappingvalvenormclosed'}
                value={
                  entry.normally_closed === null
                    ? 'N/A'
                    : entry.normally_closed
                      ? 'TRUE'
                      : 'FALSE'
                }
                class="add-config-styling"
              ...
              <div onClick={() => deleteConfigEntry(entry)}>
                <Fa icon={faTrash} color="#C53434" />
              </div>
            </div>
          )}
        </For>
      ...
      <div class="add-config-section">
        <div class="add-config-setup">
          ...
          <div style={{ 'font-weight': 'bold' }}>
            {(configurations() as Config[])[index].id}
          </div>
        </div>
        <div class="add-config-btns">
          <button class="add-config-btn" onClick={addNewConfigEntry}>
...
const DisplayConfigView: Component<{ index: number }> = (props) => {
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <div style={{ 'max-height': '100%', 'overflow-y': 'auto' }}>
        <For each={(configurations() as Config[])[index].mappings}>
          {(entry, i) => (
            <div class="add-config-configurations">
              <div
                style={{
              ...
              >
                {entry.sensor_type.replace('_', ' ').toUpperCase()}
              </div>
              <div
                style={{
                  width: '11%',
  ...
  return (
    <div class="config-view">
      ...
      <div class="system-config-above-section">
        ...
        >
          <div></div>
          <div style="text-align: center; font-size: 14px; font-family: 'Rubik'">
      ...
      <div
        class="new-config-section"
        style={{ height: ((windowHeight() - 390) as any as string) + 'px' }}
      >
        {(() => {
...
function displaySequence(index: number) {
  ...
  configDropdown.value = (sequences() as Array<Sequence>)[
    index
  ].configuration_id;
}
...
async function sendSequenceIntermediate() {
  const configDropdown = document.getElementById(
    'addassociatedconfig'
  )! as HTMLSelectElement;
  ...
  const success = (await sendSequence(
    serverIp() as string,
    currentSequnceName(),
    btoa(currentSequnceText()),
    ...
    <div class="system-sequences-page">
      ...
      <div class="sequences-list-view">
        ...
        <div style={{ 'overflow-y': 'auto', 'max-height': '150px' }}>
          <For each={sequences() as Sequence[]}>
            {(seq, i) => (
              <div
                class="sequence-display-item"
                onClick={() => displaySequence(i())}
              >
                {seq.name}
              </div>
            )}
      ...
      <div class="sequences-editor">
        <div
          style={{
            display: 'grid',
            'grid-template-columns': '240px 200px 10px 50px 1fr',
            height: '50px',
          }}
        >
          <input
            ...
            onInput={(event) =>
              setCurrentSequenceName(event.currentTarget.value)
            }
            style={{ width: '200px' }}
          />
          <div style={{ display: 'flex', 'flex-direction': 'row' }}>
            ...
            <select
              name=""
              id={'addassociatedconfig'}
              class="sequence-config-dropdown"
            >
              <For each={configurations() as Config[]}>
                {(config, i) => (
                  <option class="conf-dropdown-item" value={config.id}>
          ...
          <div></div>
          <div>
            <button class="add-config-btn" onClick={resetSequenceEditor}>
              New
            </button>
          </div>
          <div style={{ width: '100%' }}>
            <button
              style={{ float: 'right' }}
              class="submit-sequence-button"
              onClick={() => sendSequenceIntermediate()}
            >
              {saveSequenceDisplay()}
            </button>
          </div>
        ...
        <div
          class="code-editor"
          style={{ height: ((windowHeight() - 425) as any as string) + 'px' }}
        >
          <CodeMirror
            value={currentSequnceText()}
            onValueChange={(value) => {
              setCurrentSequenceText(value);
            }}
            extensions={[python()]}
            theme={oneDark}
          />
...
function resetTriggerEditor() {
  ...
  const activeDropdown = document.getElementById(
    'triggeractive'
  )! as HTMLSelectElement;
...
function displayTrigger(index: number) {
  ...
  activeDropdown.value = (triggers() as Array<Trigger>)[index]
    .active as any as string;
}
...
async function sendTriggerIntermediate() {
  ...
  const success = (await sendTrigger(
    serverIp() as string,
    currentTriggerName(),
    currentTriggerText(),
    currentConditionText(),
    JSON.parse(activeDropdown.value) as boolean
  )) as object;
  const statusCode = success['status' as keyof typeof success];
  if (statusCode != 200) {
    refreshTriggers();
  ...
  return (
    <div class="system-sequences-page">
      ...
      <div class="sequences-list-view">
        ...
        <div class="horizontal-line"></div>
        <div style={{ 'overflow-y': 'auto', 'max-height': '150px' }}>
          <For each={triggers() as Trigger[]}>
            {(trigger, i) => (
              <div
                class="trigger-display-item"
                onClick={() => displayTrigger(i())}
              >
                {' '}
      ...
      <div class="sequences-editor">
        <div
          style={{
            display: 'grid',
            'grid-template-columns': '220px 355px 150px 50px 1fr',
            height: '50px',
          }}
        >
          {' '}
          {/*Need to add active dropdown*/}
          <input
            class="connect-textfield"
            type="text"
            name="trigger-name"
            ...
            onInput={(event) =>
              setCurrentTriggerName(event.currentTarget.value)
            }
            style={{ width: '200px' }}
          />
          <div style={{ display: 'flex', 'flex-direction': 'row' }}>
            ...
            >
              <CodeMirror
                value={currentConditionText()}
                onValueChange={(value) => {
                  if (!value.includes('\n')) {
                    setCurrentConditionText(value);
                  }
                }}
                extensions={[python()]}
                theme={oneDark}
          ...
          <div>
            <button class="add-config-btn" onClick={resetTriggerEditor}>
              New
            </button>
          </div>
          <div style={{ width: '100%' }}>
            <button
              style={{ float: 'right' }}
              class="submit-sequence-button"
              onClick={() => sendTriggerIntermediate()}
            >
              {saveTriggerDisplay()}
            </button>
          </div>
        ...
        >
          <CodeMirror
            value={currentTriggerText()}
            onValueChange={(value) => {
              setCurrentTriggerText(value);
            }}
            extensions={[python()]}
            theme={oneDark}
          />
