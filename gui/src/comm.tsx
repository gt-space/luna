...
import {
  ACTIVITY_WARN_THRESH,
  DISCONNECT_ACTIVITY_THRESH,
  SERVER_PORT,
} from './appdata';
...
setInterval(() => {
  ...
  if (document.getElementById('activity') != null) {
    document.getElementById('activity')!.style.color =
      activity() < ACTIVITY_WARN_THRESH ? '#1DB55A' : '#C53434';
  }
  if (document.getElementById('status') != null) {
    document.getElementById('status')!.style.color = isConnected()
      ? '#1DB55A'
      : '#C53434';
  }
}, 10);
...
export async function afterConnect(ip: string) {
  ...
  if (isIpValid) {
    ...
    //update state
    await invoke('update_session_id', {
      window: appWindow,
      value: /*(status as AuthResponse).session_id}*/ 'session_id not in use',
    });
    await invoke('update_forwarding_id', {
      window: appWindow,
      value: 'forwarding_id not in use',
    });
    ...
    invoke('add_alert', {
      ...
      value: {
        ...
        agent: Agent.GUI.toString(),
        message: 'Connected to Servo',
      } as Alert,
    });
    result = '';
...
export async function getConfigs(ip: string) {
  try {
    const response = await fetch(
      `http://${ip}:${SERVER_PORT}/operator/mappings`,
      {
...
export async function sendActiveConfig(ip: string, config: string) {
  try {
    const response = await fetch(
      `http://${ip}:${SERVER_PORT}/operator/active-configuration`,
      {
        headers: new Headers({ 'Content-Type': 'application/json' }),
        method: 'POST',
...
export async function sendSequence(
  ...
  try {
    const response = await fetch(
      `http://${ip}:${SERVER_PORT}/operator/sequence`,
      {
        headers: new Headers({ 'Content-Type': 'application/json' }),
        method: 'PUT',
        body: JSON.stringify({
...
export async function runSequence(ip: string, name: string, override: boolean) {
  try {
    const response = await fetch(
      `http://${ip}:${SERVER_PORT}/operator/run-sequence`,
      {
        headers: new Headers({ 'Content-Type': 'application/json' }),
        method: 'POST',
        body: JSON.stringify({
...
export async function getTriggers(ip: string) {
  try {
    const response = await fetch(
      `http://${ip}:${SERVER_PORT}/operator/trigger`,
      {
        headers: new Headers({ 'Content-Type': 'application/json' }),
      }
    );
...
export async function sendCalibrate(ip: string) {
  try {
    const response = await fetch(
      `http://${ip}:${SERVER_PORT}/operator/calibrate`,
      {
        headers: new Headers({ 'Content-Type': 'application/json' }),
        method: 'POST',
      }
...
export async function stopSequence(ip: string, name: string) {
  try {
    const response = await fetch(
      `http://${ip}:${SERVER_PORT}/operator/stop-sequence`,
      {
        headers: new Headers({ 'Content-Type': 'application/json' }),
        method: 'POST',
        body: JSON.stringify({
