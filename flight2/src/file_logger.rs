...
impl std::fmt::Display for LoggerError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LoggerError::SerializationError(e) => write!(f, "Serialization error: {}", e),
            LoggerError::ChannelSendError => write!(f, "Failed to send to logging channel"),
        }
    }
}
...
impl FileLogger {
    ...
    fn writer_thread(
        ...
        let mut batch: Vec<TimestampedVehicleState> = Vec::with_capacity(config.batch_size);
        ...
        loop {
            ...
            match receiver.recv_timeout(timeout) {
                ...
                Err(mpsc::RecvTimeoutError::Disconnected) => {
                    ...
                    if !batch.is_empty() {
                        Self::write_batch(&mut current_file, &mut current_file_path, &mut batch, &mut file_size, &config);
                    }
                    break;
                }
            }
            ...
            if file_size >= config.file_size_limit {
                Self::rotate_file(&mut current_file, &mut current_file_path, &mut file_size, &config);
            }
        }
    ...
    fn open_file(path: &Path, _config: &LoggerConfig) -> Result<BufWriter<File>, std::io::Error> {
