...
impl LoggerConfig {
    /// Builds a [`LoggerConfig`] from the flight-computer CLI file-logging
    /// options (`disable-file-logging`, `log-dir`, `log-buffer-size`,
    /// `log-rotation-mb`).
...
impl LogFileBuffer {
    pub fn new(directories: Vec<PathBuf>, batch_size: usize) -> LogFileBuffer {
        LogFileBuffer {
            files: directories
                .iter()
                .map(|p| LogFile::new(&create_log_file_path(p)))
                .collect(),
            directories,
        ...
        for log_file in &mut self.files {
            if !log_file.is_open() {
                if let Err(e) = log_file.open() {
                    eprintln!(
                        "Failed to open log file {} for writing: {e}",
...
impl FileLogger {
    ...
    fn writer_thread(receiver: mpsc::Receiver<TimestampedVehicleState>, config: LoggerConfig) {
...
mod tests {
    ...
    fn generate_random_vehiclestates<F>(
        quantity: usize,
        mut closure: F,
    ) -> Vec<TimestampedVehicleState>
    where
        F: FnMut(TimestampedVehicleState),
    {
    ...
    fn open_files(parent: &Path, paths: &[&str]) -> HashMap<String, Vec<File>> {
        ...
        for path in paths {
            ...
            let mut paths: Vec<PathBuf> = walker.into_iter().map(|d| d.unwrap().path()).collect();
            paths.sort();
    ...
    fn log_write_integration(paths: &[&str], fsync_rate: usize) {
        ...
        for files in directories.values() {
            assert_eq!(
                files.len(),
                0,
                "When only writing part of the buffer, it should not write to the filesystem."
            );
        }
        ...
        for files in directories.values() {
            assert_eq!(
                files.len(),
                1,
                "Only one file should exist when the buffers first gets flushed"
            );
            ...
            assert_eq!(
                observed_states.len(),
                config.batch_size,
                "The length of the log doesn't match the expected length"
            );
            for (observed, expected) in observed_states
                .iter()
                .zip(states.iter().take(config.batch_size))
            {
                assert_eq!(
                    observed, expected,
                    "The expected state and logged state are not equal"
                );
            }
        }
        ...
        let total = (config.batch_size + config.batch_size / 2 + amount_to_rotate)
            / config.batch_size
            * config.batch_size;
        ...
        for files in directories.values() {
            assert_eq!(
                files.len(),
                2,
                "Two files should exist when the log gets rotated"
            );
    ...
    fn log_write_flush_timeout(paths: &[&str], fsync_rate: usize) {
        ...
        for files in directories.values() {
            assert_eq!(
                files.len(),
                1,
                "When writing part of the buffer with timeout, it should write to the filesystem."
            );
            let file = files.last().unwrap();
            let observed_states = deserialize_log_file(file);
