use common::comm::{
    bms,
    flight::DataMessage,
    sam::{ChannelType, SamDataPoint, Unit},
    CompositeValveState, Measurement, NodeMapping, SensorType, ValveState, VehicleState,
};
use mmap_sync::{
    locks::LockDisabled,
    synchronizer::{Synchronizer, SynchronizerError},
};
use wyhash::WyHash;
        ...
        match data_point {
            ...
            SamDataPoint::Sensor(data_point) => {
                for mapping in mappings {
                    ...
                    apply_sensor_mapping(state, mapping, data_point.channel_type, data_point.value);
                }
            }
        }
    }
