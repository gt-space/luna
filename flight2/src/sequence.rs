...
use common::comm::{flight::SequenceDomainCommand, SensorType, Sequence};

use crate::{
    common_so::{common_so_dir, python_path_for},
    Mappings,
};
...
pub(crate) fn execute(mappings: &Mappings, sequence: &Sequence, sequences: &mut Sequences) {
    if let Some(running) = sequences.get_mut(&sequence.name) {
        match running.try_wait() {
            ...
            Ok(None) => {
                println!(
                    "The '{}' sequence is already running. Stop it before re-attempting execution.",
                    sequence.name
                );
                return;
            }
