...
use crate::{
  control::{Controller, lqr::LqrController}, device::{AbortStages, Devices, Mappings}, file_logger::{FileLogger, LoggerConfig}, sensors::spawn_imu_adc_worker, sequence::Sequences, servo::ServoError, state::Ingestible
};
use clap::Parser;
use common::comm::{
  bms,
...
fn main() -> ! {
  ...
  loop {
      ...
      && Instant::now().duration_since(last_sent_to_servo) > FC_TO_SERVO_RATE
    {
      ...
      // After sending, mark GPS and RECO as consumed/invalid until a new sample
      // arrives.
