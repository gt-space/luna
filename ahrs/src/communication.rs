...
pub fn establish_flight_computer_connection(
  ...
  // Create the AHRS handshake message
  // It lets the flight computer know know what board type and number this
  // device is.
  let identity = DataMessage::Identity(AHRS_ID.to_string());

  // Allocate memory to store the AHRS handshake message in that is sent to FC
  let packet = loop {
  ...
  loop {
    // Try to send the AHRS handshake to the flight computer.
    // If this panics, it means that the AHRS couldn't send the handshake at all.
...
pub fn send_data(
  ...
  let data = DataMessage::Ahrs(AHRS_ID.to_string(), Cow::Owned(datapoint));
