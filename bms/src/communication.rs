...
use crate::{
    command::{disable_sam_power, execute},
    state::AbortInfo,
    BmsVersion, CACHED_FC_ADDRESS, FC_ADDR,
};

//const FC_ADDR: &str = "flight";
const COMMAND_PORT: u16 = 8378;
const HEARTBEAT_TIME_LIMIT: Duration = Duration::from_millis(1000);
// The amount of time we wait to hear back from the flight computer after
// disconnecting before disabling SAM power
const FC_DISCONNECT_TIMER: Duration = Duration::from_mins(20);
...
pub fn get_version() -> BmsVersion {
    ...
    let version: BmsVersion = if name == "bms-01" {
        ...
        BmsVersion::Rev3
    } else if name.starts_with("bms-4") {
        BmsVersion::Rev4
    } else {
...
pub fn send_data(socket: &UdpSocket, address: &SocketAddr, datapoint: DataPoint, hostname: String) {
