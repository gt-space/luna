...
use crate::{
    pins::{GPIO_CONTROLLERS, SPI_INFO},
...
pub fn set_estop_low() {
    let mut pin = match *BMS_VERSION {
        BmsVersion::Rev2 => GPIO_CONTROLLERS[2].get_pin(1),
        BmsVersion::Rev3 => GPIO_CONTROLLERS[1].get_pin(15),
        BmsVersion::Rev4 => {
            // P8 GPIO 69 Pin 55
...
pub fn toggle_tel(enable: bool) {
    if *BMS_VERSION == BmsVersion::Rev4 {
        ...
        eprintln!(
            "TEL load switch is only supported on BMS Rev4, but this is BMS {:?}",
            *BMS_VERSION
        );
    }
}
