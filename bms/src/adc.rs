...
pub fn init_adcs(adcs: &mut [Box<dyn ADCFamily>]) {
    for adc in adcs.iter_mut() {
        ...
        adc.set_negative_input_channel_to_aincom()
            .expect("Failed to set negative input channel to aincom");
            ...
            BmsVersion::Rev2 | BmsVersion::Rev3 => {
                if adc.kind() == VespulaBms(VespulaBmsADC::VBatUmbCharge) {
                    adc.set_positive_input_channel(0)
                        .expect("Failed to set positive input channel to 0");
                } else if adc.kind() == VespulaBms(VespulaBmsADC::SamAnd5V) {
                    adc.set_positive_input_channel(2)
                        .expect("Failed to set positive input channel to 2");
                } else {
                    panic!(
        ...
        adc.set_programmable_conversion_delay(14)
            .expect("Failed to set programmable conversion delay to 14");
        adc.set_pga_gain(1).expect("Failed to set pga gain to 1");
        adc.disable_pga().expect("Failed to disable pga");
        // datarate register
        adc.disable_global_chop()
            .expect("Failed to disable global chop");
        adc.enable_internal_clock_disable_external()
            .expect("Failed to enable internal clock disable external");
        adc.enable_continious_conversion_mode()
            .expect("Failed to enable continious conversion mode");
        adc.enable_low_latency_filter()
            .expect("Failed to enable low latency filter");
        adc.set_data_rate(4000.0)
            .expect("Failed to set data rate to 4000.0");
        // ref register
        adc.disable_reference_monitor()
            .expect("Failed to disable reference monitor");
        adc.enable_positive_reference_buffer()
            .expect("Failed to enable positive reference buffer");
        adc.disable_negative_reference_buffer()
            .expect("Failed to disable negative reference buffer");
        adc.set_ref_input_internal_2v5_ref()
            .expect("Failed to set ref input internal 2v5 ref");
        adc.enable_internal_voltage_reference_on_pwr_down()
            .expect("Failed to enable internal voltage reference on pwr down");
        // idacmag register
        adc.disable_pga_output_monitoring()
            .expect("Failed to disable pga output monitoring");
        adc.open_low_side_pwr_switch()
            .expect("Failed to open low side pwr switch");
        adc.set_idac_magnitude(0)
            .expect("Failed to set idac magnitude to 0");
        // idacmux register
        ...
        adc.disable_system_monitoring()
            .expect("Failed to disable system monitoring");
        adc.disable_spi_timeout()
            .expect("Failed to disable spi timeout");
        adc.disable_crc_byte().expect("Failed to disable crc byte");
        adc.disable_status_byte()
            .expect("Failed to disable status byte");
...
pub fn start_adcs(adcs: &mut [Box<dyn ADCFamily>]) {
    for adc in adcs.iter_mut() {
        let _ = adc.spi_start_conversion(); // start continiously collecting
                                            // data
    }
}
...
pub fn reset_adcs(adcs: &mut [Box<dyn ADCFamily>]) {
    for adc in adcs.iter_mut() {
        ...
        match *BMS_VERSION {
            BmsVersion::Rev2 | BmsVersion::Rev3 => match adc.kind() {
...
pub fn poll_adcs(adcs: &mut [Box<dyn ADCFamily>]) -> DataPoint {
    ...
    for channel in 0..6 {
        for (i, adc) in adcs.iter_mut().enumerate() {
            let reached_max_vbat_umb_charge = match *BMS_VERSION {
                BmsVersion::Rev2 => {
                    adc.kind() == VespulaBms(VespulaBmsADC::VBatUmbCharge) && channel > 4
                }
                // all input channels are used on rev3 and rev4
                BmsVersion::Rev3 | BmsVersion::Rev4 => false,
            };

            let reached_max_sam_and_5v = match *BMS_VERSION {
                BmsVersion::Rev2 | BmsVersion::Rev3 => {
