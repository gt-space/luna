...
pub fn new_temp_sensor(name: String, value: f64, unit: String) -> SensorReading {
    SensorReading {
        name,
        value, 
        unit
...
pub fn print_reading(sensor: &SensorReading, maybe_num_decimals: Option<usize>) {
    if let Some(num_decimals) = maybe_num_decimals {
        println!("{}: {:.prec$} {}", sensor.name, sensor.value, sensor.unit, prec = num_decimals);
    } else {
