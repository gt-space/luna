...
#[tokio::main]
async fn main() {
    let engine_temp_task = task::spawn(sensors::async_temp_sensor("Engine", "K"));
    let lox_temp_task = task::spawn(sensors::async_temp_sensor("LOX Temp", "K"));
    let fuel_temp_task = task::spawn(sensors::async_temp_sensor("Fuel", "K"));
    ...
    let (engine_temp, lox_temp, fuel_temp, lox_pressure, fuel_pressure, pneumatics, valve_state, _) = join!(
        engine_temp_task,
        lox_temp_task,
        fuel_temp_task,
        lox_pressure_task,
