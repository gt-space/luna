...
pub async fn async_temp_sensor(name: &str, unit: &str) -> SensorReading {
    let mut rng = StdRng::from_entropy(); //random num gen
    let value: f64 = rng.gen_range(80.0..320.0); //random temp 80-120
    sleep(Duration::from_millis(500)).await; //wait 500ms
    SensorReading {
        name: name.to_string(),
        value,
        unit: unit.to_string(),
