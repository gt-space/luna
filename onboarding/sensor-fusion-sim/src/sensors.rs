...
pub trait Sensor {
    fn read(&self) -> f64; //self refers to instance
}
...
pub fn maybe_sensor(active: bool) -> Option<u8> {
    if active { Some(12) } else { None }
}
...
pub async fn async_temp_sensor(name: &str, unit: &str) -> SensorReading {
    let mut rnd = StdRng::from_entropy(); // random number generator
    let value: f64 = rnd.gen_range(80.0..120.0); // pick random temp betweem 80 and 320
