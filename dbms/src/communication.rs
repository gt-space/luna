...
pub fn establish_flight_computer_connection(
  ...
  // Create the DBMS handshake message
  // It lets the flight computer know know what board type and number this
  // device is.
  let identity = DataMessage::Identity(DBMS_ID.to_string());

  // Allocate memory to store the DBMS handshake message in that is sent to FC
  let packet = loop {
  ...
  loop {
    // Try to send the DBMS handshake to the flight computer.
    // If this panics, it means that the DBMS couldn't send the handshake at all.
...
pub fn send_data(
  ...
  let data = DataMessage::Dbms(DBMS_ID.to_string(), Cow::Owned(datapoint));
