...
fn edit_date_time(date_time: NaiveDateTime, output: &mut DateTime<Local>, earliest: bool) -> bool {
...
fn edit_time(time: NaiveTime, output: &mut DateTime<Local>, earliest: bool) -> bool {
    edit_date_time(output.date_naive().and_time(time), output, earliest)
}

fn edit_date(date: NaiveDate, output: &mut DateTime<Local>, earliest: bool) -> bool {
...
fn parse_date_and_time(string: &str, output: &mut DateTime<Local>, earliest: bool) -> bool {
    let curr_year = format!("({:04})", output.year());
    // Full ones
    for t_fmt in TIME_FORMATS {
        for d_fmt in DATE_FORMATS {
            if let Ok(dt) =
                NaiveDateTime::parse_from_str(string, format!("{} {}", d_fmt, t_fmt).as_str())
            {
