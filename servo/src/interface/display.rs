...
use common::comm::{CompositeValveState, Measurement, ValveState};
use crossterm::{
...
impl<T: Clone> StringLookupVector<T> {
    ...
    /// Sorts the backing vector by name, meaning iterating through this
    /// structure will go through alphabetical.
...
async fn update_information(tui_data: &mut TuiData, shared: &Shared, system: &mut System) {
    async fn update_source_pane(
        ...
        for (ref board_name, ref stats) in vehicle_state.rolling {
            if !pane.system_data.contains_key(board_name) {
                pane.system_data.add(board_name, SystemDatapoint::default());
            }
    ...
    async fn update_source_stats(
        ...
        if let Some(last_update) = *telemetry.last_vehicle_state.0.lock().await {
            datapoint.value.time_since_update = Some(last_update.elapsed().as_secs_f64() * 1000.0);
        }
    ...
    .await;
    update_source_stats(&radio_name, TelemetrySource::Radio, tui_data, shared).await;
    ...
    update_source_pane(&mut tui_data.umbilical, shared, TelemetrySource::Umbilical).await;
...
fn draw_empty(f: &mut Frame, area: Rect) {
    ...
    let empty_table: Table<'_> = Table::new(Vec::<Row>::new(), widths)
        .style(YJSP_STYLE)
        .header(Row::new(vec![Span::from("").into_centered_line()]).style(Style::new().bold()));
        ...
        if let Some(cpu_usage) = datapoint.cpu_usage {
            rows.push(
                Row::new(vec![
                    ...
                    Cell::from(Span::from(format!("{:.1}", cpu_usage)).into_right_aligned_line()),
                    Cell::from(Span::from("%")),
                ])
                .style(data_style),
            );
        ...
        if let Some(mem_usage) = datapoint.mem_usage {
            rows.push(
                Row::new(vec![
                    ...
                    Cell::from(Span::from(format!("{:.1}", mem_usage)).into_right_aligned_line()),
                    Cell::from(Span::from("%")),
                ])
                .style(data_style),
            );
            ...
            rows.push(
                Row::new(vec![
                    Cell::from(Span::from("Last Update").into_right_aligned_line())
                        .style(data_style),
                    Cell::from(Span::from(handle_last_update.clone()).into_right_aligned_line()),
    ...
    for name_datapoint_pair in source_systems.iter() {
        ...
        if let Some(time_since_update) = &datapoint.time_since_update {
            rows.push(
                Row::new(vec![
                    Cell::from(Span::from("Last Update").into_right_aligned_line()),
                    Cell::from(
                        Span::from(format!("{:.3}", time_since_update)).into_right_aligned_line(),
                    ),
                    Cell::from(Span::from("ms")),
                ])
                .style(data_style),
        ...
        if let Some(update_rate) = &datapoint.update_rate {
            rows.push(
                Row::new(vec![
                    Cell::from(Span::from("Update Rate").into_right_aligned_line()),
                    Cell::from(Span::from(format!("{:.3}", update_rate)).into_right_aligned_line()),
                    Cell::from(Span::from("Hz")),
                ])
                .style(data_style),
            );
...
/// See update_information for how this data is gathered
fn draw_valves(f: &mut Frame, area: Rect, pane: &TelemetryPaneData, selected_source_label: &str) {
    ...
    for pair in full_valves.iter() {
        ...
        let voltage_rows = if datapoint.knows_voltage {
            [
                Cell::from(
                    Span::from(format!("{:.2}", datapoint.voltage)).into_right_aligned_line(),
                ), // Voltage
                Cell::from(Span::from(format!("{:+.3}", d_v)).into_right_aligned_line())
                    .style(d_v_style),
            ]
        } else {
            [Cell::from(""), Cell::from("")]
        ...
        let current_rows = if datapoint.knows_current {
            [
                Cell::from(
                    Span::from(format!("{:.3}", datapoint.current)).into_right_aligned_line(),
                ), // Current
                Cell::from(Span::from(format!("{:+.3}", d_i)).into_right_aligned_line())
        ...
        rows.push(
            Row::new(vec![
                ...
                Cell::from(Span::from(format!("{}", datapoint.state.actual)).into_centered_line())
                    .style(get_state_style(datapoint.state.actual)),
                // Commanded state of valve
                Cell::from(
                    Span::from(format!("{}", datapoint.state.commanded)).into_centered_line(),
                )
...
/// See update_information for how this data is gathered
fn draw_sensors(f: &mut Frame, area: Rect, pane: &TelemetryPaneData, selected_source_label: &str) {
