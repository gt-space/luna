...
use common::comm::{
    AbortStageConfig, Computer, FlightControlMessage, NodeMapping, Sequence, Trigger,
    ValveSafeState, VehicleState, VehicleStateCompaqError,
};
use jeflog::warn;
...
use super::{
    telemetry::{update_live_telemetry, TelemetrySource},
    Database, Shared,
};
    ...
    pub async fn send_all_abort_configs(&mut self) -> anyhow::Result<()> {
        let stages = self
            ...
            .query_map([], |row| {
                ...
                let valve_safe_states = postcard::from_bytes::<HashMap<String, ValveSafeState>>(
                    &bytes,
                )
                .map_err(|error| {
...
pub fn receive_vehicle_state(shared: &Shared) -> impl Future<Output = io::Result<()>> {
    ...
    async move {
        ...
        loop {
            match recv_vehicle_state_packet(&socket, &mut frame_buffer, &mut control_buffer) {
                Ok((datagram_size, source, _)) => {
                    ...
                    match new_state {
                        Ok(state) => {
                            update_live_telemetry(telemetry.get(source), state, datagram_size)
                                .await;
                        }
                        Err(error) => warn!(
...
fn telemetry_source_from_control(
    ...
    while !current.is_null() && (current as usize).saturating_add(size_of::<libc::cmsghdr>()) <= end
    {
        let header = unsafe { &*current };
        if header.cmsg_level == libc::IPPROTO_IP && header.cmsg_type == libc::IP_TOS {
            let data = unsafe { (current as *const u8).add(cmsg_data_offset()).cast::<u8>() };
            let tos = unsafe { *data };
            let dscp = tos >> 2;
...
fn format_radio_decode_error(error: VehicleStateCompaqError) -> String {
    match error {
        VehicleStateCompaqError::Schema(_) | VehicleStateCompaqError::PolicyDesynchronized => {
