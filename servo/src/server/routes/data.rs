...
async fn telemetry_source_stats(telemetry: &LiveTelemetry) -> TelemetrySourceStats {
...
pub async fn export(
    ...
    let vehicle_states = database
        ...
        .query_map([request.from, request.to], |row| {
            ...
            let vehicle_state = postcard::from_bytes::<VehicleState>(&bytes).map_err(|error| {
    ...
    match request.format.as_str() {
        "csv" => {
            ...
            for (timestamp, state) in vehicle_states {
                ...
                content += &format!(
                    ",{},{}",
                    state.fc_sensors.barometer.temperature, state.fc_sensors.barometer.pressure,
                );
                content += &format!(
                    ",{},{}",
                    state.fc_sensors.adc.rail_5v.voltage, state.fc_sensors.adc.rail_3v3.voltage,
                );
        ...
        "hdf5" => {
            ...
            let path = servo_dir
                .join((String::from("ExportFile") + &file_index + &String::from(".hdf5")).as_str());
...
/// Route function which returns the latest telemetry transport stats for all
/// sources.
pub async fn telemetry_stats(State(shared): State<Shared>) -> Json<TelemetryStatsResponse> {
...
mod tests {
    ...
    use common::comm::{
        bms::Bms, fc_sensors::FcSensors, sam::Unit, AbortStage, CompositeValveState, Measurement,
        ValveState,
    };
        ...
        for _ in 0..8 {
            ...
            // automatically deconstruct and close.
            {
                ...
                let metadata_group = file
                    .group("metadata")
                    .expect("HDF5 file for data exports should always have metadata group in it");
                ...
                let valves_group = file
                    .group("valves")
                    .expect("HDF5 file for data exports should always have valve folder in it");
                for i in 0..4 {
                ...
                let sensors_group = file
                    .group("sensors")
                    .expect("HDF5 file for data exports should always have sensor folder in it");
                for i in 0..4 {
