...
/// Register address to read the number of available bytes (high byte at 0xFD,
/// low byte at 0xFE)
...
impl GPS {
    ...
    /// # Arguments
    ///
    /// * `i2c_bus` - I2C bus number (typically 1 for /dev/i2c-1 on Raspberry
    ///   Pi)
    ...
    /// # Arguments
    ///
    /// * `meas_rate_ms` - Measurement period in milliseconds (e.g., 50 for 20
    ///   Hz, 100 for 10 Hz)
    /// * `nav_rate` - Navigation rate (number of measurement cycles per
    ///   navigation solution, typically 1)
    ...
    /// Configures the module to send periodic NAV-PVT messages at a specified
    /// rate
    ///
    /// # Arguments
    ///
    /// * `rate` - Rate for each port [DDC, UART1, UART2, USB, SPI, Reserved]
    ///   For I2C (DDC), typically set rate[0] to desired value (e.g., 1 for
    ///   every solution)
    ///
    ...
    /// This is useful in periodic mode where the module automatically sends
    /// NAV-PVT messages.
    ///
    /// # Returns
    ///
    ...
    /// bytes from registers 0xFD/0xFE, then reading exactly that many bytes
    /// from the data stream register 0xFF. This avoids wasting time reading
    /// a large fixed-size buffer full of 0xFF when little or no data is
    /// available.
    fn read_packets<T: FnMut(PacketRef)>(&mut self, mut cb: T) -> Result<bool, GPSError> {
        ...
        self.i2c
            .write_read(&[UBLOX_NUM_BYTES_REG], &mut count_buf)?;

        // The datasheet specifies MSB at 0xFD and LSB at 0xFE, so this is
        // big-endian.
