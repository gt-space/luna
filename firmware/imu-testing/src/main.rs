...
fn main() {
    ...
    let mut imu_cs = GPIO_CONTROLLERS[IMU_CS_PIN_LOC[0]].get_pin(IMU_CS_PIN_LOC[1]);
    imu_cs.mode(Output);
    let mut bar_cs = GPIO_CONTROLLERS[BAR_CS_PIN_LOC[0]].get_pin(BAR_CS_PIN_LOC[1]);
    bar_cs.mode(Output);
    let mut mag_cs = GPIO_CONTROLLERS[MAG_CS_PIN_LOC[0]].get_pin(MAG_CS_PIN_LOC[1]);
    mag_cs.mode(Output);
    let mut imu_dr = GPIO_CONTROLLERS[IMU_DR_PIN_LOC[0]].get_pin(IMU_DR_PIN_LOC[1]);
    imu_dr.mode(Input);
    let mut imu_nreset = GPIO_CONTROLLERS[IMU_NRESET_PIN_LOC[0]].get_pin(IMU_NRESET_PIN_LOC[1]);
    ...
    // Get spi
    let spi = Spidev::open("/dev/spidev0.0").expect("Spi initialization should work");

    // Initialize the actual spi handler
    if let Ok(mut driver) = AdisIMUDriver::initialize(spi, imu_dr, imu_nreset, imu_cs) {
