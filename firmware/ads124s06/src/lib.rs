...
use common::comm::{
    ...
    ADCError, ADCFamily, ADCKind,
};
use spidev::{spidevioctl::SpidevTransfer, SpiModeFlags, Spidev, SpidevOptions};
...
impl ADCFamily for ADC {
    ...
    fn new(
        ...
        let drdy_pin_boxed = drdy_pin.map(|pin| Box::new(pin) as Box<dyn GpioPin>);
        let cs_pin_boxed = cs_pin.map(|pin| Box::new(pin) as Box<dyn GpioPin>);
...
impl ADC {
    ...
    fn spi_read_data(&mut self) -> Result<i32, ADCError> {
        ...
        match result {
            Ok(_) => {
                let mut value =
                    ((rx_buf[1] as i32) << 16) | ((rx_buf[2] as i32) << 8) | (rx_buf[3] as i32);
