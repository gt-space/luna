...
use common::comm::{ADCKind::{self, SamAnd5V, VBatUmbCharge}, DataPoint, ChannelType, Pin, PinValue::{High, Low}};
...
impl<'a> ADC<'a> {
  ...
  pub fn poll(&mut self, i: u8) -> Option<DataPoint> {
    let reached_max_vbat_umb_charge = self.kind == VBatUmbCharge && i > 4;
    let reached_max_sam_and_5v = self.kind == SamAnd5V && i < 2;
    ...
    // poll for data ready
    loop {
      let drdy_val = self.drdy_pin.digital_read();
      if drdy_val == Low {
        break;
      }
    }
    ...
    // call spi read data
    let raw_code = match self.spi_read_data() {
      Ok(data) => data,
      Err(e) => {
    ...
    // do shit with data
    let data = self.calculate_differential_measurement(raw_code);

    if self.kind == VBatUmbCharge {
      self.set_positive_input_channel((i + 1) % 5).ok();
    }

    if self.kind == SamAnd5V {
      if i == 5 {
        self.set_positive_input_channel(0).ok();
      } else {
        self.set_positive_input_channel(i + 1).ok();
      }
    }
