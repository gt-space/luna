...
#[derive(Debug)]
pub enum Error {
    /// Indicates that the most recently dispatched conversion command has
    /// failed.
    ///
    ...
    /// Indicates that the PROM CRC check failed, signalling that the
    /// calibration data has been corrupted.
...
impl MS5611 {
    /// Constructs a new `MS5611` given the path to the device file
    /// corresponding to the barometer's SPI bus.
    ///
    ...
    pub fn new_with_gpio_pin(bus: &str, cs: Option<Box<dyn GpioPin>>, osr: u16) -> Result<Self> {
    ...
    /// Reads the ROM on MS5611's internal barometer containing calibration
    /// data.
    ...
    /// calling this method, then the conversion will be performed in the
    /// method, which may take up to 9 ms.
    pub fn read_temperature(&mut self) -> Result<f64> {
        ...
        let mut off = ((self.prom.off_t1 as i64) << 16) + ((self.prom.tco as i64 * dt) >> 7);
