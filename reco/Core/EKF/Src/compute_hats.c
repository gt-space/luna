...
void compute_wn(float32_t phi, float32_t h, float32_t vn, float32_t ve, arm_matrix_instance_f32* wn, float32_t we, float32_t buffer[3] PERF_ARG) {

	float32_t computeRadiiVec[4];
	compute_radii(phi, computeRadiiVec PERF_PASS);
...
void compute_what(arm_matrix_instance_f32* q, arm_matrix_instance_f32* bias_g, arm_matrix_instance_f32* sf_g,
	...
	PERF_START(2);
	compute_wn(phi, h, vn, ve, &wn, we, wnBuff PERF_PASS);
	PERF_END(PERF_WN, 2);
	...
	PERF_START(3);
	quaternion2DCM(q, &D_bn, D_bn_buff PERF_PASS);
	PERF_END(PERF_QUAT2DCM, 3);
