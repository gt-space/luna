...
void compute_dpdot_dp(float32_t phi, float32_t h, float32_t vn, float32_t ve, arm_matrix_instance_f32* dpdot_dp, float32_t dpDotBuff[9] PERF_ARG) {

	PERF_START(1);

    float32_t computeRadiiResult[4];
    compute_radii(phi, computeRadiiResult PERF_PASS);
	...
	PERF_END(PERF_COMPUTE_DPDOT_DP, 1);
}
 ...
 */
void compute_dpdot_dv(float32_t phi, float32_t h, arm_matrix_instance_f32* dpdot_dv, float32_t dpDotBuff[9] PERF_ARG) {
	...
	PERF_END(PERF_COMPUTE_DPDOT_DV, 1);
}
...
void compute_dvdot_dp(float32_t phi, float32_t h, float32_t vn, float32_t ve, float32_t vd,
	...
	PERF_START(2);
    compute_g_dg2(phiRad, h, gDgResult PERF_PASS);
	PERF_END(PERF_COMPUTE_G_DG2, 2);
	...
	PERF_END(PERF_COMPUTE_DVDOT_DP, 1);
}
...
void compute_dvdot_dv(float32_t phi, float32_t h, float32_t vn, float32_t ve, float32_t vd,
	...
	PERF_END(PERF_COMPUTE_DVDOT_DV, 1);
}
...
void compute_F(arm_matrix_instance_f32* q, arm_matrix_instance_f32* sf_a, arm_matrix_instance_f32* sf_g,
	...
	quaternion2DCM(q, &D_nb, dnbBuff PERF_PASS);
...
//	arm_mult_f32(offsetResultData, wMeasBiasGDiff, wMeasBiasGDiff, 3);
	...
	compute_dwdp(phi, h, ve, vn, we, &dwdp, dwdpBuff PERF_PASS);
	...
	compute_dwdv(phi, h, &dwdv, dwdvBuff PERF_PASS);
	...
	compute_dpdot_dp(phi, h, vn, ve, &F22, dpdot_dpData PERF_PASS);
	compute_dpdot_dv(phi, h, &F23, dpdot_dvData PERF_PASS);
	...
	compute_ahat(q, sf_a, bias_a, a_meas, &ahat_n, ahat_nData PERF_PASS);
	arm_mat_scale_f32(&D_bn, -1.0f, &D_bn);
	...
	compute_dvdot_dp(phi, h, vn, ve, vd, we, &F32, dvdot_dpData PERF_PASS);
	compute_dvdot_dv(phi, h, vn, ve, vd, we, &F33, dvdot_dvData PERF_PASS);
...
void compute_G(arm_matrix_instance_f32* sf_g, arm_matrix_instance_f32* sf_a, arm_matrix_instance_f32* q,
	...
	quaternion2DCM(q, &D_bn, DbnBuff PERF_PASS);
