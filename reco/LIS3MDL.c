...
uint8_t generateMagAddress(mag_reg_t magRegNum, bool readFlag, bool consecutiveFlag) {
...
inline uint8_t ensureMagNotReserved(mag_reg_t regToCheck) {
...
short writeMagRegister(spi_device_t* magSPI, mag_reg_t magRegNum, uint8_t valueToWrite) {

	/*
	ensureMagNotReserved(magRegNum);
	*/

	uint8_t actualAddress = generateMagAddress(magRegNum, false, false);
...
uint8_t readMagSingleRegister(spi_device_t* magSPI, mag_reg_t magRegNum) {
	...
	*/
	...
	uint8_t actualAddress = generateMagAddress(magRegNum, true, false);
	uint8_t regValue;
...
uint16_t readMagDoubleRegister(spi_device_t* magSPI, mag_reg_t upperRegAddress, mag_reg_t lowerRegAddress) {
	...
	*/
	...
	ensureMagNotReserved(upperRegAddress);
	ensureMagNotReserved(lowerRegAddress);

	uint8_t upper8 = readMagSingleRegister(magSPI, upperRegAddress);
	uint8_t lower8 = readMagSingleRegister(magSPI, lowerRegAddress);
...
uint8_t* readMagMultipleRegisters(spi_device_t* magSPI, mag_reg_t magRegNum, uint8_t numRegisters) {
	...
	*/
	...
	magRegNum = generateMagAddress(magRegNum, true, true);
...
uint8_t initializeMag(spi_device_t* magSPI, mag_handler_t* magHandler) {

	uint8_t status = writeMagRegister(magSPI, MAG_CTRL_REG1, magHandler->CTRL_REG_FLAGS[0]);
	status = writeMagRegister(magSPI, MAG_CTRL_REG2, magHandler->CTRL_REG_FLAGS[1]);
	status = writeMagRegister(magSPI, MAG_CTRL_REG3, magHandler->CTRL_REG_FLAGS[2]);
	status = writeMagRegister(magSPI, MAG_CTRL_REG4, magHandler->CTRL_REG_FLAGS[3]);
	status = writeMagRegister(magSPI, MAG_CTRL_REG5, magHandler->CTRL_REG_FLAGS[4]);
...
uint8_t getStatusRegister(spi_device_t* magSPI) {
