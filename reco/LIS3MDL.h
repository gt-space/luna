...
} CTRL_REG4_CONFIG_BITS;

typedef enum {
	MAG_CTRL5_FAST_READ 		  = (1 << 7),
	MAG_CTRL5_BDU				  = (1 << 6)
} CTRL_REG5_CONFIG_BITS;
...
uint8_t* readMagMultipleRegisters(spi_device_t* magSPI, mag_reg_t regAddress, uint8_t numRegisters);
uint8_t readMagSingleRegister(spi_device_t* magSPI, mag_reg_t regAddress);
uint16_t readMagDoubleRegister(spi_device_t* magSPI, mag_reg_t upperRegAddress, mag_reg_t lowerRegAddress);
short writeMagRegister(spi_device_t* magSPI, mag_reg_t regAddress, uint8_t valueToWrite);
