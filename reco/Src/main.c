...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	...
	if (htim->Instance == TIM13) {
		...
		if (convertTemp) {
			printf("Entering pressure. Press: %f Pa. Temp: %f C, dT: %d C.\n", baroHandler->pressure, baroHandler->temperature, baroHandler->dT);
			calculatePress(baroSPI, baroHandler);
			startTemperatureConversion(baroSPI, baroHandler);
 			convertTemp = false;
 			printf("Leaving pressure Press: %f Pa. Temp: %f C, dT: %d C.\n\n", baroHandler->pressure, baroHandler->temperature, baroHandler->dT);
		} else {
			printf("Entering temperature. Press: %f Pa. Temp: %f C, dT: %d C.\n", baroHandler->pressure, baroHandler->temperature, baroHandler->dT);
			calculateTemp(baroSPI, baroHandler);
			startPressureConversion(baroSPI, baroHandler);
			convertTemp = true;
			atomic_fetch_add(&baroEventCount, 1);
			printf("Leaving temperature. Press: %f Pa. Temp: %f C, dT: %d C.\n\n", baroHandler->pressure, baroHandler->temperature, baroHandler->dT);
		}
